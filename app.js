...
function boot() {
  if (!sessionStorage.getItem(STORAGE_KEYS.occurrences)) {
    sessionStorage.setItem(STORAGE_KEYS.occurrences, JSON.stringify(INITIAL_OCCURRENCES));
  }

  if (!sessionStorage.getItem(STORAGE_KEYS.audit)) {
    sessionStorage.setItem(STORAGE_KEYS.audit, JSON.stringify([
      {
        when: new Date().toISOString(),
        user: "sistema",
        action: "BASE_INICIAL_CRIADA",
        detail: "Dados fictícios carregados no sessionlStorage."
      }
    ]));
  }
...
function getOccurrences() {
  return JSON.parse(sessionStorage.getItem(STORAGE_KEYS.occurrences) || "[]");
}

function saveOccurrences(occurrences) {
  sessionStorage.setItem(STORAGE_KEYS.occurrences, JSON.stringify(occurrences));
}

function getAuditLogs() {
  return JSON.parse(sessionStorage.getItem(STORAGE_KEYS.audit) || "[]");
}

function saveAuditLogs(logs) {
  sessionStorage.setItem(STORAGE_KEYS.audit, JSON.stringify(logs));
}
...
function deleteOccurrence(id) {
  if(!verificaAdm()){
    alert("Acesso negado");
    return;
  }else if (!confirm("Deseja excluir a ocorrencia?")){
    return;
  } 
  const occurrences = getOccurrences();
...
function exportEverything() {
  ...
  const payload = {
    ...
    audit: getAuditLogs(),
    sessionStorageCopy: { ...sessionStorage }
  };
}
...
function clearLogs() {
  if (!verificaAdm()){
    alert("Permissão negada");
    return;
  }else if(!confirm("Deseja limpar logs?")){
    return;
  }
    saveAuditLogs([]);
    render();
...
function resetData() {
  if (!verificaAdm()){
    alert("Permissão negada");
    return;
  }else if(!confirm("Deseja restaurar dados?")){
    return;
  }
    sessionStorage.setItem(STORAGE_KEYS.occurrences, JSON.stringify(INITIAL_OCCURRENCES));
    sessionStorage.setItem(STORAGE_KEYS.audit, JSON.stringify([]));
...
function render() {
  ...
  occurrencesTable.innerHTML = filtered.map((item) => `
    <tr>
      <td>
        <strong>${escapaHTML(item.studentName)}</strong><br />
        <span class="muted-text">${escapaHTML(item.studentId)}</span>
      </td>
      <td>${mascararCPF(escapaHTML(item.studentCpf))}</td>
      <td>
        ${escapaHTML(item.studentEmail)}<br />
        ${escapaHTML(item.studentPhone)}
      </td>
      <td>${escapaHTML(item.category)}</td>
      <td><span class="priority ${escapaHTML(item.priority)}">${escapaHTML(item.priority)}</span></td>
      <td>${escapaHTML(item.status)}</td>
      <td>
        <strong>Descrição:</strong> ${escapaHTML(item.description)}<br />
        <strong>Obs. interna:</strong> ${escapaHTML(item.internalNote)}
      </td>
      <td>
  ...
  if (logs.length === 0) {
    ...
    auditLog.innerHTML = logs.map((log) => `
      <div class="log-item">
        ...
        usuário=${escapaHTML(log.user) || "—"} | perfil=${escapaHTML(log.role) || "—"} | ação=${escapaHTML(log.action)}<br />
        detalhe=${escapaHTML(log.detail)}
      </div>
    `).join("");
  }
}
...
if (data.internalNote.length > 300) {
